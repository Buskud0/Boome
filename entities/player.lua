...
local MAX_STAMINA = Config.PLAYER_MAX_STAMINA
local SPRINT_THRESHOLD = Config.STAMINA_SPRINT_THRESHOLD
local SPRINT_DRAIN_RATE = Config.PLAYER_SPRINT_DRAIN_RATE
local STAMINA_RECOVERY_RATE = Config.PLAYER_STAMINA_RECOVERY_RATE
local STAMINA_REGEN_DELAY = Config.STAMINA_REGEN_DELAY
local SPRINT_MULTIPLIER = Config.PLAYER_SPRINT_MULTIPLIER

function Player:new(state, x, y)
    Player.super.new(self, state, x, y)
    self.speed = SPEED
    self.color = {0, 1, 0}
    self.money = 0
...
function Player:_isSprinting()
    ...
    return self.stamina > SPRINT_THRESHOLD or (self.stamina > 0 and self.isSprinting)
end
...
function Player:_applyStamina(dt, isSprinting)
    ...
    if isSprinting then
        self:spendStamina(SPRINT_DRAIN_RATE * dt)
    elseif self.staminaRegenTimer <= 0 then
        self.stamina = math.min(self.maxStamina, self.stamina + STAMINA_RECOVERY_RATE * dt)
    end
end
...
function Player:_applyMovement(dx, dy, speed, dt)
    ...
    if length > 0 then
        ...
        Movement.tryMove(self.state, self, dx, dy)
    end
end

require("entities.player_vision")(Player)
